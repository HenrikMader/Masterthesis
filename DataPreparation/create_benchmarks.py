...
def select_last_20_percent(group):
    # Determine how many rows make up the last 20%
    num_rows = len(group)
    last_20_percent = int(num_rows * 0.2)
    
    # Select the last 20% of rows
    return group.tail(last_20_percent)
...
def remainTimeOrClassifBenchmark(dataset, path, file_name, start_date, end_date, max_days, test_len_share, 
    ...
    dataset["remain_time"] = dataset.groupby("case:concept:name")["time:timestamp"].transform(lambda x: (x.max() - x).dt.total_seconds() / (60 * 60 * 24))
